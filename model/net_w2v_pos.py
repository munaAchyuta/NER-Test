...
def flat_accc(preds,labels):
    #print(preds)
    pred_flat = np.array(preds).flatten()#preds.cpu().numpy().flatten()
    ...
    # indexing with negative values is not supported. Since PADded tokens have label -1, we convert them to a positive
    # number. This does not affect training, since we ignore the PADded tokens with the mask.
    labels = labels % len(preds[0])#preds.shape[1]

    #num_tokens = int(torch.sum(mask).data.item())

    #print(np.sum(pred_flat == labels_flat),"======", len(labels_flat))
    labels_flat = labels.flatten()
    labels_flat = list(labels_flat.cpu().numpy())
    #print("$$$ : ",labels_flat)
    #print("&&& : ",pred_flat)
